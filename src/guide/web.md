---
description: Node.js 웹 애플리케이션 정적분석, 포멧터, 테스트 코드 적용하기
...
✅ [ESLint](https://eslint.org/)와 [Prettier](https://prettier.io/)를 이용하여 정적 코드 분석을 합니다.  
...
`Awesome API`는 `Fastify` 웹 프레임워크를 이용하여 개발중입니다. 아직 초창기라 많은 코드를 작성하진 못했지만 `/`에 접속하면, 완벽하게 `{ "hello": "world" }`를 응답합니다.
...
<Chat-KakaoRoom>
  ...
  <Chat-KakaoMsg msg="오타라던가 사용하지 않는 변수등을 체크해서 잠재적인 버그를 찾을 수 있게 도와줍니다" isMe="false" />
...
`Where does your code run?` 설정은 복수 응답을 허용하기 때문에 `space`키를 눌러 `node`만 체크하고 엔터를 입력합니다.
:::
...
`app.js`파일의 5번째 줄에 선언했으나 사용하지 않는 변수 2개가 발견되었습니다. 변수를 제거하고 다시 `lint`를 실행합니다.
...
<Chat-KakaoRoom>
  ...
  <Chat-KakaoMsg avatar="senior" user="촋 CTO" msg="띄어쓰기 라던가 따옴표 형태, 세미콜론 사용 유무등 사용자마다 선호하는 방식이 다른 코딩 습관을 동일한 기준에 맞춰 수정할 수 있어요" isMe="false" />
  <Chat-KakaoMsg msg="아하! 안그래도 저희끼리 규칙을 정하고 있었어요!" isMe="true" />
...
`app.js`파일과 `server.js`파일을 분석한 결과를 확인할 수 있습니다. 작은 따옴표(')가 전부 큰 따옴표(")로 변경되었고 세미콜론을 빼먹은 부분도 추가되었네요. 문자랑 연산자 사이에 적당한 간격이 생겼습니다.

스크립트에서 변경된 사항을 바로 수정하도록 `--write` 옵션을 추가합니다.
...
ESLint와 Prettie를 따로 실행하지 않고 한번에 실행하면 좋겠죠? `package.json`에 스크립트를 추가합니다.
...
이제 `npm run validate`로 두가지 모두 체크할 수 있습니다.
...
✅ [Visual Studio Code formatOnSave, formatOnPaste 설정하기](https://code.visualstudio.com/docs/getstarted/settings)
:::

## Jest

사실 개발팀은 고민이 하나 있습니다. 지속 가능한 프로젝트의 성장을 위해선 테스트코드를 작성하라고 들었는데, 엄두가 나지 않았던 거죠.
...
<Chat-KakaoRoom>
  ...
  <Chat-KakaoMsg msg="아! 우선 단위 테스트를 작성해보죠. 일단 한번 해보고 더 자세히 알아볼께요!" isMe="false" />
  <Chat-KakaoMsg msg="Javascript엔 다양한 테스트 프레임워크가 있는데 이번엔 Jest를 사용할께요. 아주 빠르고 확장성이 좋아요" isMe="false" />
</Chat-KakaoRoom>
...
Jest는 기본적으로 `__tests__` 디렉토리나 `*.test.js` 파일을 바라봅니다.
...
테스트를 실행하도록 `package.json`에 다음 내용을 추가합니다.
...
이거 정말 정상적으로 동작한걸까요? 의심스러우니 코드 내용을 바꿔서 다시 실행해봅니다.
...
나중에 소스코드가 변경되면 테스트 코드도 같이 변경해야 하는걸까요? -> 맞습니다! 하지만, 매번 변경된 내용을 복사 붙여넣기 하긴 어렵죠.

Jest에서는 `toMatchInlineSnapshot` 함수를 지원합니다.

기존 테스트 코드를 다음과 같이 수정합니다.
...
다시 코드를 `hey?`에서 `world`로 변경하면, 테스트가 실패하고 Snapshot을 업데이트 할 지 물어봅니다.
...
정상적으로 변경한 것이므로 Snapshot을 업데이트 합니다.

```sh
npm t -- -u
```

매번 복사 붙여넣기할 필요가 없어서 편하네요. Snapshot기능은 API 테스트를 작성하거나 프론트엔트 렌더링을 확인할 때 유용하게 사용할 수 있습니다.
...
정적 코드 분석, 포멧팅, 테스트 코드는 프로젝트 초기엔 그 장점을 잘 느낄수 없지만, 프로젝트가 성장할수록 안정적이고 빠르게 개발할 수 있는 안전망 역할을 합니다.

이제 기본적인 설정을 마쳤으니 본격적으로 협업을 위한 설정을 알아봅니다.
